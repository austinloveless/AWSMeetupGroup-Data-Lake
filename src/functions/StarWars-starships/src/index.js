...
exports.handler = async (event) => {
  return getStarWarsStarships();
};

// gets star wars starships
...
const putRawS3Data = async (data) => {
  ...
  const params = {
    ...
    Key: `raw/starwars/raw_starships/${timestamp}.json`,
  };
  try {
...
const transformedData = async (data) => {
  ...
  const params = {
    Body: JSON.stringify(data),
    Bucket: process.env.s3Bucket,
    Key: `transformed/starwars/transformed_starships/${timestamp}.json`,
  };
